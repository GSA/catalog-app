...
function test_login_and_datasets () {
  ...
  curl --silent --fail 'http://ckan:5000/login_generic?came_from=/user/logged_in' \
    --compressed \
    -H 'Content-Type: application/x-www-form-urlencoded' \
    -H 'Origin: http://ckan:5000' \
    -H 'Referer: http://ckan:5000/user/login' \
    --data 'login=ckan_admin&password=test1234' \
    --cookie-jar ./cookie-jar

  dataset_success=$(curl --fail --location --request GET 'http://ckan:5000/api/3/action/package_show?id=test-dataset-1' --cookie ./cookie-jar | grep -o '"success": true')
...
function check_google_id () {
  ...
  find_id=$(curl --silent --fail --request GET 'http://ckan:5000' | grep -o "$google_id")
...
@test "/user/login is up" {
  curl --silent --fail http://ckan:5000/user/login
}
...
@test "data is inaccessible to public" {
  run curl --fail --location --request GET 'http://ckan:5000/api/3/action/package_show?id=test-dataset-1'
  # Validate output is 22, curl response for 403 (Forbidden)
  [ "$status" -eq 22 ]
}

@test "Website display is working" {
  curl --silent --fail http://ckan:5000/dataset/test-dataset-1 --cookie ./cookie-jar
}

@test "Google Analytics ID present" {
  # check_google_id
  echo "Skipping google analytics test"
}
